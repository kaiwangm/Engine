...
void Octree::insert(uint32_t x, uint32_t y, uint32_t z, uint32_t r, uint32_t g,
    ...
    for (uint32_t level = 0; level <= m_MaxLevel; ++level) {
        ...
        real_coords[level] = {coords[level][0] * div + div / 2,
                              coords[level][1] * div + div / 2,
                              coords[level][2] * div + div / 2};
    ...
    for (uint32_t level = 0; level <= m_MaxLevel; ++level) {
        OctreeNode::OctreeNodeType type =
            level == m_MaxLevel ? OctreeNode::OctreeNodeType::Leaf
                                : OctreeNode::OctreeNodeType::Branche;

        std::array<uint32_t, 3> real_coord_cur = real_coords[level];
        std::array<uint32_t, 3> real_col_cur = real_cols[level];
        if (level == 0) {
            if (ptr == nullptr) {
                auto new_node = std::make_shared<OctreeNode>(
                    real_coord_cur[0], real_coord_cur[1], real_coord_cur[2],
                    real_col_cur[0], real_col_cur[1], real_col_cur[2], level, 0,
                    type);
                m_Root = new_node;
                ptr = new_node;
            ...
            if (nex_ptr == nullptr) {
                auto new_node = std::make_shared<OctreeNode>(
                    real_coord_cur[0], real_coord_cur[1], real_coord_cur[2],
                    real_col_cur[0], real_col_cur[1], real_col_cur[2], level,
                    octant, type);
