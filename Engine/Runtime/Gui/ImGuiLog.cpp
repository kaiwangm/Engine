...
namespace ImGui
{
    ...
    void ShowExampleAppLog(bool* p_open)
    {
        ...
        static size_t        pos_read = 0;

        std::string s      = Engine::Log::GetOString()->str();
        size_t      last_n = s.size();

        if (last_n != pos_read)
        {
            s        = s.substr(pos_read, last_n - pos_read);
            pos_read = last_n;
        ...
        if (ImGui::SmallButton("Click me to add a log"))
        {
            Engine::Log::Info("I am a small button.");
        }
        ImGui::End();
