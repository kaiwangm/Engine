...
namespace Engine
{
    ...
    public:
        MTriangleShader(const std::string& name) : MMaterial(name, "TriangleShader") { m_Color = glm::vec3(1.0f); }

        ~MTriangleShader() = default;

        void Bind(const Ref<Shader> shader) { shader->SetFloat3("color", m_Color); }
