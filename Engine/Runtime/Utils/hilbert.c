...
static void
getIEEEinitValues(double const* c1, unsigned y, unsigned nDims, unsigned* rotation, bitmask_t* bits, bitmask_t* index)
{
    ...
    /* find the position of the least-order 0 bit in among signBits and adjust
       it if necessary */
...
int hilbert_cmp(unsigned nDims, unsigned nBytes, unsigned nBits, void const* c1, void const* c2)
{
    ...
    return hilbert_cmp_work(
        nDims, nBytes, nBits, 0, nBits, (char const*)c1, (char const*)c2, 0, bits, bits, getIntBits);
}
...
int hilbert_ieee_cmp(unsigned nDims, double const* c1, double const* c2)
{
    ...
    return hilbert_cmp_work(
        nDims, 8, 64, max, max + 53, (char const*)c1, (char const*)c2, rotation, bits, index, getIEEEBits);
}
...
unsigned hilbert_box_vtx(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, void* c1, void* c2)
{
    bitmask_t const one  = 1;
    bitmask_t       bits = one << (nDims - 1);
    return hilbert_box_vtx_work(
        nDims, nBytes, nBits, findMin, 0, nBits, (char*)c1, (char*)c2, 0, bits, bits, getIntBits);
}
...
unsigned hilbert_ieee_box_vtx(unsigned nDims, int findMin, double* c1, double* c2)
{
    ...
    return hilbert_box_vtx_work(
        nDims, 8, 64, findMin, max, max + 53, (char*)c1, (char*)c2, rotation, bits, index, getIEEEBits);
}
...
unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, void* c1, void* c2)
{
    bitmask_t const one  = 1;
    bitmask_t       bits = one << (nDims - 1);
    return hilbert_box_pt_work(
        nDims, nBytes, nBits, findMin, 0, nBits, (char*)c1, (char*)c2, 0, bits, bits, getIntBits, propogateIntBits);
}
...
unsigned hilbert_ieee_box_pt(unsigned nDims, int findMin, double* c1, double* c2)
{
    ...
    return hilbert_box_pt_work(nDims,
                               8,
                               64,
                               findMin,
                               max,
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
int hilbert_nextinbox(unsigned    nDims,
                      unsigned    nBytes,
                      unsigned    nBits,
                      int         findPrev,
                      void*       c1V,
                      ...
                      void const* ptV)
{
    ...
    while (y-- > 0)
    {
        ...
        {
            /* compute (the complement of) a "digit" in the integer index of
               this point */
        ...
        for (r = 0; r < nPrints; ++r)
        {
            ...
            for (r1 = 0; r1 < r; ++r1)
            {
                ...
                if (ans != 1)
                {
                    ...
                    printf("cmp r = 0x%0*x; r1 = 0x%0*x, ans = %2d\n", width, (unsigned)r, width, (unsigned)r1, ans);
                }
            }
            hilbert_i2c(nDims, nBits, r1, coord1);
...
int main()
{
    ...
    hilbert_box_vtx(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1, cornerlo, work);
    ...
    hilbert_box_vtx(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 0, work, cornerhi);
...
int cmp(void const* c1p, void const* c2p) { return hilbert_ieee_cmp(g_nDims, c1p, c2p); }
...
int main()
{
    ...
    hilbert_box_pt(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1, pointlo, work);
    ...
    hilbert_box_pt(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 0, work, pointhi);
