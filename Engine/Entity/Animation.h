...
struct JointTransform {
    ...
    static JointTransform Interpolate(const JointTransform& frameA,
                                      const JointTransform& frameB,
                                      const float progression) {
        glm::vec3 position =
            glm::mix(frameA.Position, frameB.Position, progression);
        glm::quat rotation =
            glm::slerp(frameA.Rotation, frameB.Rotation, progression);
...
struct KeyFrame {
    ...
    KeyFrame(const float timeStamp,
             const std::unordered_map<std::string, JointTransform>& pose)
...
class Animator {
   public:
    ...
    std::unordered_map<std::string, JointTransform> GetCurrentPose() {
   ...
   private:
    ...
    void IncreaseAnimationTime(const float timeStep) {
        ...
        if (m_AnimationTime > m_CurrentAnimation->GetTimeLength()) {
            m_AnimationTime =
                std::fmod(m_AnimationTime, m_CurrentAnimation->GetTimeLength());
        }
    }
    ...
    float CalCulateProgression(const KeyFrame& previousFrame,
    ...
    std::unordered_map<std::string, JointTransform> InterpolatePoses(
        const KeyFrame& previousFrame, const KeyFrame& nextFrame,
        float progression) {
        std::unordered_map<std::string, JointTransform> currentPose;
        for (const auto& [jointName, transform] : previousFrame.Pose) {
            JointTransform previousTransform =
                previousFrame.GetJointTransform(jointName);
            JointTransform nextTransform =
                nextFrame.GetJointTransform(jointName);

            currentPose[jointName] = JointTransform::Interpolate(
...
class AnimatedModel {
   public:
    ...
    AnimatedModel(const std::string& path)
        ...
        for (int i = 0; i < scene->mNumAnimations; ++i) {
            ...
            std::unordered_map<std::string,
                               std::vector<std::pair<JointTransform, float>>>
                poses;
            ...
            for (int j = 0; j < animation->mNumChannels; ++j) {
                ...
                for (int k = 0; k < nodeAnimation->mNumPositionKeys; ++k) {
                    ...
                    poses[jointName].push_back(
                        {JointTransform(
                             glm::vec3(posKey.mValue.x, posKey.mValue.y,
                                       posKey.mValue.z),
                             glm::quat(rotKey.mValue.w, rotKey.mValue.x,
                                       rotKey.mValue.y, rotKey.mValue.z)),
                         posKey.mTime});
                }
            }
            ...
            DoAnimation(std::make_shared<Animation>(keyFrames.back().TimeStamp,
                                                    keyFrames));
        }
    }

    void processJoint(aiNode* node, Ref<Joint> praentJoint) {
        praentJoint->m_LocalBindTransform =
    ...
    Mesh processMesh(aiMesh* amesh, const aiScene* scene) {
        ...
        for (int i = 0; i < amesh->mNumBones; ++i) {
            ...
            if (m_JointMapping.find(boneName) == m_JointMapping.end()) {
                ...
                m_Joints.push_back(std::make_shared<Joint>(boneIndex, boneName,
                                                           glm::mat4(1.0f)));
            } else {
    ...
    void Draw(Ref<Shader> shader, glm::mat4 vpMat, glm::mat4 transform) {
        for (const auto& mesh : m_Meshes) {
            ...
            ApplyPoseToJoints(currentPose, m_RootJoint->m_Children[0],
                              glm::mat4(1.0f));
            ...
            for (auto joint : m_Joints) {
                ...
                std::string paraName =
                    "jointTransforms[" + std::to_string(jointIdx) + "]";
   ...
   public:
    ...
    void addJointsToVector(const Ref<Joint>& joint,
