...
namespace Engine
{
    ...
    public:
        ...
        void TickRender() override
        {
            ...
            m_Octrees[nowFrame]->RenderOctree(nowLevel, m_ShaderLibrary.Get("OctreeShader"));
            ...
            Gui::SliderFloat3("m_Camera_Position", m_Camera->GetPosition(), -10.0f, 10.0f);
            Gui::SliderFloat("m_Camera_Rotation", m_Camera->GetRotation(), -180.0f, 180.0f);

            uint32_t nowLevel_min = 0;
            uint32_t nowLevel_max = 12;
            ImGui::SliderScalar("nowLevel", ImGuiDataType_U32, &nowLevel, &nowLevel_min, &nowLevel_max);

            uint32_t nowFrame_min = 0;
            uint32_t nowFrame_max = m_Octrees.size() - 1;
            ImGui::SliderScalar("nowFrame", ImGuiDataType_U32, &nowFrame, &nowFrame_min, &nowFrame_max);
            ...
            ImGui::Begin("ViewPort :: Color");
            {
                ...
                // from the UV.
                m_FrameRenderBuffer->SetViewPort((uint32_t)wsize.x, (uint32_t)wsize.y);
                ImGui::Image(m_FrameRenderBuffer->GetTextureID(), wsize, ImVec2(0, 1), ImVec2(1, 0));
                ImGui::EndChild();
            }
            ImGui::End();
